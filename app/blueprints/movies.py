...
# Logic to UPDATE movie to DB route 
@movies.route('/<string:imdb_id>', methods=['PUT'])
def updateMovie(imdb_id):

    # parse the request data
    data = request.get_json()

    # Get movie in db
    existingMovie = Movie.query.filter_by(imdb_id=imdb_id).first()

    # If movie can't be found, return error
    if not existingMovie:
        return jsonify({'error': 'Movie not found'}), 404
    
    # Update movie poster
    setattr(existingMovie, 'poster', data['poster'])

    # Commit vhange
    db.session.commit()

    # Return success message    
    return jsonify({'message': 'Movie updated successfully', 'movie': existingMovie.to_dict()})